...
// Appendix B), shared by HPACK (HTTP/2) and QPACK (HTTP/3). Callers use
package huffman
...
// O(len(src)) via nybble FSM (DECODE_TABLE).
decode :: proc(dst: ^[dynamic]u8, src: []u8) -> Error {
	state: u8 = 0
	for byte_ in src {
		// High nybble then low nybble (MSB-first bit order).
		hi := DECODE_TABLE[u16(state) * 16 + u16(byte_ >> 4)]
		if hi.flags & DECODE_FAIL != 0 do return .Invalid_Code
		if hi.flags & DECODE_EOS != 0 do return .EOS_In_Input
		if hi.flags & DECODE_SYM != 0 {
			append(dst, hi.sym)
		}
		state = hi.state

		lo := DECODE_TABLE[u16(state) * 16 + u16(byte_ & 0x0F)]
		if lo.flags & DECODE_FAIL != 0 do return .Invalid_Code
		if lo.flags & DECODE_EOS != 0 do return .EOS_In_Input
		if lo.flags & DECODE_SYM != 0 {
			append(dst, lo.sym)
		}
		state = lo.state
	}
	if !DECODE_STATE_ACCEPT[state] do return .Invalid_Padding
	return .None
}

// Linear bit-walk decoder (original algorithm). Kept for unit-test cross-check
// against the FSM path; not used on the production decode path.
decode_slow :: proc(dst: ^[dynamic]u8, src: []u8) -> Error {
