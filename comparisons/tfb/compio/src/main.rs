//! Plain text + HTML on compio (io_uring driver on Linux).
//! Minimal HTTP/1.1 — network path uses the completion runtime.
...
fn route(req_head: &str) -> Vec<u8> {
    ...
    if line.starts_with("GET /fortunes") {
        http_response("200 OK", "text/html; charset=utf-8", &fortunes_body())
