...

{
  ...
  echo "mechanisms:"
  echo "  proactr-mat: tiny/gen/blob=materialize_copy assembled=preconcat_blob file=file_read_full sse=sse_oneshot(chunked TE stream)"
  echo "  proactr-opt: assembled=multi_send (NOT kernel_writev/writev) file=file_chunked (NOT kernel_sendfile) tiny/gen=materialize"
  echo "  laytan/ntex/drogon: assembled=preconcat_blob (NOT multi_send) file=file_read_full sse=sse_oneshot(CL body)"
