// Microbench: product package quic (OpenSSL dynlib)
package main
...
main :: proc() {
	...
	// pure AEAD reused (cell C)
	{
		keys: q.Initial_Keys
		dcid := DCID
		if !q.derive_initial_keys(&keys, dcid[:]) {
			emit("pure_aead_seal_1200", packet_iters, 0, false)
			return
		}
		plain := make([]u8, 1200)
		defer delete(plain)
		...
		for i in 0 ..< warmup_p {
			nonce[0] = u8(i)
			_, _ = q.aead_seal(&keys.client, out, nonce[:], plain, aad[:])
		}
		t0 := time.tick_now()
		for i in 0 ..< packet_iters {
			nonce[0] = u8(i)
			_, ok := q.aead_seal(&keys.client, out, nonce[:], plain, aad[:])
			if !ok { emit("pure_aead_seal_1200", packet_iters, 0, false); return }
		}
		...
		defer delete(pt)
		for _ in 0 ..< warmup_p {
			_, _ = q.aead_open(&keys.client, pt, nonce[:], out[:n], aad[:])
		}
		t0 = time.tick_now()
		for _ in 0 ..< packet_iters {
			_, ook := q.aead_open(&keys.client, pt, nonce[:], out[:n], aad[:])
			if !ook { emit("pure_aead_open_1200", packet_iters, 0, false); return }
		}
		emit("pure_aead_open_1200", packet_iters, time.duration_nanoseconds(time.tick_since(t0)), true)
	}

	// full Initial seal (reused keys — openssl path)
	{
		keys: q.Initial_Keys
		dcid := DCID
		_ = q.derive_initial_keys(&keys, dcid[:])
		scid := [8]u8{1, 2, 3, 4, 5, 6, 7, 8}
		plain: [1000]u8
		out: [2048]u8
		for i in 0 ..< 1000 do plain[i] = u8(i)
		for i in 0 ..< warmup_p {
			_, _ = q.encrypt_initial(out[:], dcid[:], scid[:], nil, u64(i), 4, plain[:], &keys.client)
		}
		t0 := time.tick_now()
		for i in 0 ..< packet_iters {
			_, ok := q.encrypt_initial(out[:], dcid[:], scid[:], nil, u64(i), 4, plain[:], &keys.client)
			if !ok { emit("initial_seal", packet_iters, 0, false); return }
		}
	...
	{
		for _ in 0 ..< warmup_h {
			if !loopback_once() { emit("loopback_handshake", hs_iters, 0, false); return }
		}
		t0 := time.tick_now()
		for _ in 0 ..< hs_iters {
			if !loopback_once() { emit("loopback_handshake", hs_iters, 0, false); return }
